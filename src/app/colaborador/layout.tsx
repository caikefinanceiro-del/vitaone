...
import { COLABORADOR_NAV } from "@/lib/navigation";
...
export default function ColaboradorLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen bg-background text-on-background font-body-md overflow-hidden">
      <Sidebar items={COLABORADOR_NAV} className="hidden md:flex" />

      {mobileSidebarOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          <div className="absolute inset-0 bg-black/40" onClick={() => setMobileSidebarOpen(false)} />
          <div className="absolute left-0 top-0 h-full w-[260px] bg-surface shadow-xl animate-slide-in">
            <Sidebar items={COLABORADOR_NAV} className="flex w-full" isMobile={true} />
          </div>
        </div>
      )}
